...
import com.airplayer.airplay.handshake.AirPlayNtpClient
import com.airplayer.airplay.handshake.AudioStreamServer
import com.airplayer.airplay.handshake.BufferedAudioServer
import com.airplayer.airplay.handshake.MirrorStreamServer
 ...
 *
 * HOW: [AirPlayerService] creates this receiver and calls [start]/[stop].
 ...
 *
 * State changes are reported via [onStateChanged] to [AirPlayerService].
 *
 * Example:
...
class AirPlayReceiver(
     ...
     * ([AirPlayerService]) uses this to update the [ActiveConnection] and notification
    ...
    private val pairingStore = com.airplayer.airplay.handshake.PairingStore(context)
     ...
     *
     * MUST be called when [AirPlayerService] stops or is destroyed.
     */
    fun stop() {
    ...
    private fun onStreamingStarted(session: SessionDescription) {
        ...
        scope.launch {
            try {
                ...
                emitNowPlaying()
                // Notify AirPlayerService of the sender name BEFORE emitting CONNECTED,
    ...
    /**
     * Called from [MirrorStreamServer]'s watchdog thread when the video pipe never resumed after a
     * surface regain (TV screensaver dismissed, but macOS's mirror data connection stayed silent).
     * Forces the control connection closed so the receiver falls back to ADVERTISING immediately
     * instead of sitting on a black screen that still reports CONNECTED — the same recovery a manual
     * disconnect/reconnect from macOS already achieves, just automatic.
     */
    private fun onMirrorStalled() {
        Logger.w("Mirror stream stalled — forcing a full session reset so the sender can reconnect")
        rtspHandler?.closeActiveConnection()
    }
