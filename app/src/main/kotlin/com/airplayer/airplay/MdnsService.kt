...
import com.airplayer.util.NetworkUtils

/**
 * MdnsService — Advertises AirPlayer as an AirPlay 2 receiver on the local network.
 *
 * WHY: For macOS/iOS to show AirPlayer in the AirPlay menu, the device must announce
 * itself using mDNS (Multicast DNS, the same protocol as Apple's Bonjour).
 * Without this advertisement, no sender would know AirPlayer exists.
 *
...
class MdnsService(
     ...
     * on the network is already registered as "AirPlayer", Android will register us as
     * "AirPlayer (2)" instead. The [onActualNameRegistered] callback delivers the name
     * that was actually registered (which may differ from the requested name).
     *
     * The caller can use this to update the UI (e.g., show "Registered as: AirPlayer (2)")
     ...
     *
     * TXT records tell senders what features AirPlayer supports.
