 ...
 *
 * WHY: MdnsService is the gateway between AirPlayer and macOS discovery.
 * If the mDNS registration is wrong (wrong service type, missing TXT records),
 * macOS will never show AirPlayer in the AirPlay menu. These tests verify that
...
class MdnsServiceTest {
     ...
     * device stays in the menu even when AirPlayer is not running.
     */
    @Test
     ...
     * macOS won't be able to connect to AirPlayer.
     */
    @Test
    ...
    fun `stop emits DISABLED protocol state`() {
        val states = mutableListOf<com.airplayer.service.ProtocolState>()
        val service = MdnsService(mockContext, onStateChange = { states.add(it) })

        service.stop()

        assertTrue(states.contains(com.airplayer.service.ProtocolState.DISABLED))
    }
