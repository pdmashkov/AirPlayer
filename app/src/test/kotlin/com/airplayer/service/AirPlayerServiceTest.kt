...
/**
 * AirPlayerServiceTest — Unit tests for [AirPlayerService].
 *
 * WHY: [AirPlayerService] is the central coordinator. Bugs in its state aggregation
 ...
 *
 * HOW: [AirPlayerService] is an Android [Service] and requires Android APIs for
 ...
 */
class AirPlayerServiceTest {

    // ─── Companion constants ──────────────────────────────────────────────────

    @Test
    fun `ACTION_START has package-qualified value`() {
        assertEquals("com.airplayer.action.START", AirPlayerService.ACTION_START)
    }

    @Test
    fun `ACTION_STOP has package-qualified value`() {
        assertEquals("com.airplayer.action.STOP", AirPlayerService.ACTION_STOP)
    }

    @Test
    fun `ACTION_RESTART has package-qualified value`() {
        assertEquals("com.airplayer.action.RESTART", AirPlayerService.ACTION_RESTART)
    }
    ...
    fun `NOTIFICATION_ID is positive`() {
        assertTrue(
            ...
            AirPlayerService.NOTIFICATION_ID > 0
        )
    }

    @Test
    fun `CHANNEL_ID is non-empty`() {
        assertTrue(AirPlayerService.CHANNEL_ID.isNotEmpty())
    }

    @Test
    fun `all three ACTION constants are distinct`() {
        val actions = setOf(
            AirPlayerService.ACTION_START,
            AirPlayerService.ACTION_STOP,
            AirPlayerService.ACTION_RESTART
        )
    ...
    /** The exact mapping lambda from [AirPlayerService.startAirPlay] — tested as a pure function. */
    ...
    // wiring between AirPlayerService and MainActivity without needing Android.
